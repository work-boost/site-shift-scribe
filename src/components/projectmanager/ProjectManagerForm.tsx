...
const ProjectManagerForm: React.FC<ProjectManagerFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let sstImageUrl = null;
      
      if (sstImage) {
        sstImageUrl = await uploadSstImage(sstImage);
        if (!sstImageUrl) {
    ...
    <Card className="border-2 border-indigo-300 shadow-xl">
      <CardHeader className="bg-gradient-to-r from-indigo-500 to-blue-500 text-white rounded-t-lg">
        <div className="flex justify-between items-center">
          <CardTitle className="text-2xl font-bold">Add Project Manager</CardTitle>
          <Button 
            variant="ghost" 
            size="sm" 
            onClick={onCancel}
